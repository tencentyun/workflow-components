...
def parse_argument():
    ret = {}
    gitUrl = os.environ.get('GIT_CLONE_URL', None)
    if not gitUrl:
        return {}
    ...
    if version:
        ret['version'] = version
    # 暂时不提供
    rcfile = os.environ.get('RCFILE', None)
    if rcfile:
        ret['rcfile'] = rcfile

    outPutType = os.environ.get('OUT_PUT_TYPE', None)
    if outPutType:
        ret['out-put-type'] = outPutType
